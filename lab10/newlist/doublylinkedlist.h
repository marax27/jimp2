...
public:
	...
	void push_back(bool elem){
		...
		BitRef b = BitRef(&tail->data, tail->number_used);
		b = elem;
		++tail->number_used;
	}
	...
	Node next(const Node &node) const {
		if(node.index == node.base->number_used - 1)
	...
	Node previous(const Node &node) const {
		if(!node.index)
			return Node(node.base->prev, node.base->prev->number_used - 1);
		else
			return Node(node.base, node.index - 1);
	}
